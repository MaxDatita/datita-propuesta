...
interface PrismaticBurstProps {
  intensity?: number;
  speed?: number;
  animationType?: 'rotate3d' | 'rotate' | 'hover';
  colors?: string[];
  distort?: number;
  paused?: boolean;
  ...
  rayCount?: number;
  mixBlendMode?: 'lighten' | 'multiply' | 'screen' | 'overlay' | 'none';
}
...
const PrismaticBurst = ({
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      ro?.disconnect();
      if (!ro && typeof window !== 'undefined') (window as Window).removeEventListener('resize', resize);
      io?.disconnect();
      ...
      try {
        if (meshRef.current) {
                  // @ts-expect-error - remove method exists on Mesh
